...
def main():
    ...
    adapter = LmStudioAdapter(
        base_url="http://localhost:1234/v1", models=["qwen/qwen3.5-9b"]
    )
    ...
    orch.tool_registry.register(
        "read_file", read_file, [], "Reads a file from the workspace"
    )
    orch.tool_registry.register(
        "edit_file", edit_file, ["write"], "Edits a file using patch format"
    )

    messages = [
        {
            "role": "user",
            "content": "Read target.txt, then use edit_file to change 'apples' to 'oranges'. You must strictly use the edit_file tool.",
        }
    ]
    ...
    for step in range(5):
        ...
        res = orch.run_agent_once(
            system_prompt_name="operational", messages=current_messages, tools={}
        )
        ...
        if last_msg["role"] == "user" and "tool_execution_result" in last_msg.get(
            "content", ""
        ):
