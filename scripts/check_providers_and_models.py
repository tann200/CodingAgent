...
from src.core.llm_manager import get_provider_manager, get_available_models, call_model, canonical_provider
...
async def run_check(provider_override: str | None = None):
    ...
    try:
        ...
        if pmodels:
            ...
            if isinstance(pmodels, list) and pmodels:
                first = pmodels[0]
                configured = first if isinstance(first, str) else (first.get('id') or first.get('key') or first.get('name'))
    except Exception:
        configured = None
    ...
    system_prompt = system_prompt_file.read_text(encoding='utf-8') if system_prompt_file.exists() else "You are a helpful local coding assistant."
    ...
    try:
        resp = await call_model(messages, provider=p_key, model=chosen, stream=False, format_json=False)
