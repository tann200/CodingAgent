...
def build_messages(system_prompt_path: str, user_prompt: str):
    sys_text = Path(system_prompt_path).read_text(encoding='utf-8') if Path(system_prompt_path).exists() else ''
    messages = [
...
async def main():
    ...
    try:
        resp = await call_model(messages, provider=args.provider, model=args.model, stream=False, format_json=False)
        print('Response:')
        print(resp)
