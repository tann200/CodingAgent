...
- **Property/fuzz tests are deterministic and dependency-free** — `tests/unit/test_property_based_fuzzing.py` covers the pure pipeline (token estimator/truncation, both output pruners, prompt-injection guard, byte-cap truncation, alias/kind maps, `_path_inside`) with `random.Random(0xC0FFEE)`-seeded loops (150 iterations each). No hypothesis or external fuzz framework — a failure is fully reproducible from the fixed seed. Add new property cases there rather than spinning up a separate framework.
- **Benchmark tests live in `tests/benchmarks/`** — `test_pipeline_benchmarks.py` (dispatch/planning/DAG) and `test_pure_hotpath_benchmarks.py` (truncation, pruning, permission/registry lookup, prompt-injection guard, typed MessageBus dispatch). They run in the dedicated CI `benchmarks` job (schedule / dispatch / main), never in `tests/unit`, so they don't affect the unit baseline. Thresholds are intentionally generous regression guards, not SLAs — keep them deterministic and LLM-free. (See PHASE-4 4.4.)
- **`prune_tool_outputs` metadata guard** — non-dict `metadata` values are treated as not-preserve (`isinstance(meta, dict) and meta.get("preserve")`); do not revert to `msg.get("metadata", {})` which crashes on string/list metadata. (Caught by the 4.3 fuzz suite.)
- **Perception node is thin orchestration** — `perception_node.py` is a delegating orchestrator, not a logic dump: each `perception_*.py` helper module is self-contained (module-level `logger` + deps resolved in-module via graceful try/except imports). Do not re-parameterize helpers with injected `logger=`/`fn=`/`cls=` DI params. The only sanctioned test seams are the `call_model_fn` param of `_validate_call_model_and_adapter` and the `call_model` module attribute of `perception_node` (patched by integration/e2e tests). Legacy test surface is preserved via `# noqa: F401` re-exports; adding local copies of helper logic (e.g. a second YAML parser) is a regression. (See PHASE-4 4.1.)
...
- **Autonomous approval REQUIRES an operator allowlist (HS-1/Phase 5)** — `is_autonomous()` alone never suppresses approval prompts. The single decision point is `autonomous_approval_allowed(name)` in `tools_config.py`: it resolves tool aliases to canonical names and honors `set_autonomous_approve()` / `configure(autonomous_approve=...)` / the `CODINGAGENT_AUTONOMOUS_APPROVE` env var (comma-separated names or `*` for all gated tools). Empty allowlist = fail-closed: every DANGER/PROMPT tool is DENIED in autonomous mode, never silently approved. All four suppression sites (`tool_execution_pipeline._run_permission_gate`, `tool_execution_service._check_permission_gate`, `permission_gateway._gate5_user_approval`, `_bash_exec._check_tier3_approval`) must consult it — do not re-add a bare `if autonomous: allow` anywhere.
...
- **Per-tool network policy must be explicit and enforceable (MC-6/Phase 5)** — every tool that routes through `run_sandboxed()` must pass `network=` explicitly (both `bash()` and `bash_readonly()` pass `network=False`); never rely on the parameter default. `_check_network_policy` in `_bash_exec.py` refuses network-capable commands (classified by `is_network_capable` in `_approval.py`: curl/wget/pip/apt/brew/ssh/rsync/scp + multi-token git remote ops, npm/cargo/go install, etc.) when the deny cannot be enforced (sandbox level `"off"` or no enforcing backend → unsandboxed fallback) and `_enforcement_required()` is true (autonomous / `SANDBOX_REQUIRE_ENFORCEMENT`); interactive mode runs them with a `system.warning` event. New network-using tools must be classified in the network-capable sets, not inline. (See MC-6.)
- **`run_in_background` must never silently escape the sandbox (TW-2/Phase 5)** — the background branch of `bash()` bare-`Popen`s its command; any new background path must go through `_build_background_sandbox` (`_bash_exec.py`). That helper wraps the argv in bwrap (`--unshare-net`, ro system binds, `--die-with-parent`) or an enforcing sandbox-exec profile; when it returns `None` the spawn is only permitted if `_enforcement_required()` is false (interactive) and must publish a `system.warning` — refuse otherwise. Keep output `DEVNULL` (an inherent cap). (See TW-2.)
...
- **Single canonical token estimator** — `estimate_text_tokens(text)` lives ONLY in `src/core/context/token_truncation.py` (wraps `tokenizer.count_tokens`, tiktoken/HF → len//3.5 fallback, with a `len // 4` last-resort guard). `tool_output_truncation.py` imports it rather than defining a private copy (PHASE-4 4.2 removed the duplicate); `token_truncation.truncate_*` default to it, so pruning and truncation agree on token counts. Do not re-add a second estimator; the memory subsystem's deterministic `len//4` helpers are deliberately separate (docs say "matches Rust"). (See audit 2.4 / PHASE-4 4.2.)
- **TUI mirror trees `tui/src` and `tui/tui_src` must stay byte-identical (Item-2)** — Runtime imports `tui.tui_src` (`src/main.py` P1-3 renamed it so the package name is unique and no sys.path shadowing is needed), while `scripts/run_tui.py`, `python -m tui.src.ui.main`, and a few unit tests import `tui.src`. Both roles are load-bearing, so the two trees are maintained as mirrors, NOT consolidated. After editing one tree, mirror the change to the other, and enforce parity with `TestMirrorSync::test_src_and_tui_src_trees_byte_identical` (`tests/unit/test_core_bridge_decomposition.py`) plus `diff -rq tui/src tui/tui_src --exclude=__pycache__ --exclude='*.pyc'`. Note that files inside `tui/src` self-import `tui.tui_src` (e.g. `ui/mock_engine.py`), so `tui.src` is not a standalone importable package.
- **Permission-gateway kind fallback is drift-locked** — `permission_gateway._TOOL_KIND_MAP` is a pure no-registry fallback keyed by canonical tool names; `_tool_kind_for_name()` resolves aliases via `resolve_tool_alias` first, and `test_tool_kind_map_keys_resolve_to_live_registry_tools` fails if a map key no longer resolves to a tool in the built-in registry. Keep keys canonical (never stale legacy names such as `glob_tool`/`list_dir`/`run_bash`). (See PHASE-4 4.2.)
