...
def main():
    ...
    orch.tool_registry.register(
        "edit_file", dummy_edit_file, ["write"], "Edits a file with a patch"
    )

    messages = [
        {
            "role": "user",
            "content": "Please read 'src/main.py' and tell me what you find. Just execute the tool call.",
        }
    ]

    print("\n--- Running agent loop ---")
    res = orch.run_agent_once(
        system_prompt_name="operational", messages=messages, tools={}
    )
