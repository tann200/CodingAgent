...
**Status:** Security cluster (2.1/2.2/2.8) + observability counter (2.3) complete; next task is **2.5**.
...
Tests: `tests/unit/test_phase2_security_hardening.py` (13 tests) + 2 new counter tests in `test_state_schemas.py`. mypy + ruff check clean.
...
| 2.5 | Guard per-round debug serialization | Low | `perception_node.py:939` — `repr(resp)[:1000]` runs even when logging disabled. Simple `if logger.isEnabledFor(...)` guard. Quick win. |
| 2.6 | Remove duplicate `_is_success()` | Low | `execution_routing.py:25`, `perception_routing.py:209`. Simple dedup. Quick win. |
| 2.7 | Centralize routing magic numbers | Medium | All routing modules. Single source of truth for thresholds. |

**Suggested ordering after 2.3:** next is **2.5** and then **2.6** (trivial quick wins), then **2.7**, then **2.4** as a standalone high-complexity effort.
