...
try:
    ...
    def lm_save_provider(data, path=None, initial_path=None):
        try:
            target = Path(initial_path) if initial_path else Path(path or 'config/providers.json')
...
class OllamaAdapter(LLMClient):
    DEFAULT_TIMEOUT = LM_DEFAULT_TIMEOUT
    def __init__(self, config_path=None, name=None, base_url=None, api_key=None):
        ...
        if isinstance(self.provider, list) and len(self.provider) > 0:
            ...
            for p in self.provider:
                if str(p.get('type')).lower() == 'ollama' or str(p.get('name')).lower() == 'ollama':
                    found = p
                    break
            self.provider = found or self.provider[0]

        # If base_url provided by caller, prefer that and consider provider present
        self.base_url = base_url or (self.provider.get('base_url') if self.provider else 'http://localhost:11434/api')
        ...
        self.models = (self.provider.get('models') if self.provider and isinstance(self.provider.get('models'), list) else [])
    ...
    def _save_provider(self):
        return lm_save_provider(self.provider, self.config_path, self._initial_config_path)
    ...
    def get_models_from_api(self):
            ...
            try:
                ...
                for item in models_raw:
                    if isinstance(item, dict):
                        raw_key = item.get('name') or item.get('model') or item.get('id') or item.get('key')
                        name = str(raw_key).split('/')[-1] if raw_key is not None else None
                        if name:
        ...
        print('Error: Ollama service not reachable or returned unexpected response for tags endpoints')
        return {'models': []}
    ...
    def _generate_internal(self, prompt, model=None, stream=False, format_json=False, **kwargs):
        ...
        # when JSON-format requested, request `format": "json"` unless caller provided it
        if format_json and "format" not in payload:
            payload["format"] = "json"
        try:
            if stream:
                # Use a streaming-compatible direct call to requests.post so test fakes receive stream=True
                try:
                ...
                response = self._call_requests('post', url, json=payload, stream=stream, timeout=60)
            if isinstance(response, dict) and response.get('meta'):
                # _call_requests returned an error dict
                return response
            response.raise_for_status()
            if stream:
                # Return a generator expression that yields decoded JSON objects or raw chunks
                def _stream_gen(resp):
                    for line in resp.iter_lines():
                        if not line:
                            continue
                        try:
                            # iter_lines may already yield str in some fakes
                ...
                data = response.json()
                if format_json:
                    # Try to parse the model's response text as JSON
                    parsed = self._parse_json_response_field(data.get("response"))
                    # If parsing failed or response empty, try meta fields that may contain the structured output
                    if (parsed is None or parsed == "" or parsed == data.get("response")):
                        meta = data.get('meta', {}) if isinstance(data.get('meta', {}), dict) else {}
                        # common fallback keys: 'thinking', 'thoughts', or top-level 'thinking'
                        fallback_text = None
                    ...
                    return {
                         ...
                         "response": parsed,
                         "meta": {k: v for k, v in data.items() if k not in ("model", "created_at", "response")}
                     }
                return data
        except requests.exceptions.ConnectionError:
    ...
    def _chat_internal(self, messages, model=None, stream=False, format_json=False, timeout=None, **kwargs):
        """Chat with the model using messages (list of {role, content}).
        Returns parsed response; if format_json=True attempts to parse assistant content as JSON.
        ...
        try:
            ...
            if stream:
                ...
                data = response.json()
                # Chat responses nest the assistant message under 'message'
                message = data.get("message") or {}
                content = message.get("content") if isinstance(message, dict) else None
                parsed = self._parse_json_response_field(content) if format_json else content
                # Fallback: if no content or parsing empty, try meta.thinking or top-level 'thinking'
                if format_json and (parsed is None or parsed == "" or parsed == content):
                    meta = data.get('meta', {}) if isinstance(data.get('meta', {}), dict) else {}
                    fallback_text = None
                    if isinstance(meta, dict):
                        fallback_text = meta.get('thinking') or meta.get('thoughts')
                    ...
                    if fallback_text:
                        ...
                        if isinstance(parsed_fb, (dict, list)):
                            parsed = parsed_fb
                # Ensure that returned structure includes parsed response for runner use
                # Additionally, if format_json was False but the content is a JSON string, attempt to parse it so callers get structured data
                ...
                result = {
                    ...
                    "message": message,
                    "response": parsed,
                    "meta": {k: v for k, v in data.items() if k not in ("model", "created_at", "message")}
                }
                # Guarantee we always return a dict and at least a 'meta' key so callers can inspect errors
    ...
    def extract_tool_calls(self, chat_response):
        """Extract tool call requests from a chat response.
        Accepts the returned dict from chat() and returns a list of tool call dicts (if any).
        """
        message = chat_response.get("message") if isinstance(chat_response, dict) else None
        if not message:
            return []
        # Ollama may include tool_calls in the message
        tool_calls = message.get("tool_calls") or message.get("tool_calls", [])
        if tool_calls:
            return tool_calls
        # Some models may encode tool call as JSON in content
    ...
    def _do_request(self, method: str, url: str, **kwargs):
        """Generic request method; wraps requests.get/requests.post etc.

        - retries on connection error (with backoff)
        - integrates with adapter-level timeout
        - can be overridden in subclasses for custom behavior
        """
        # Timeout is always applied, even if not explicitly set (defaulting to 10s)
        # This ensures that all requests respect the adapter-level timeout setting.
        # Note: we don't use requests' built-in timeout=... here to avoid confusion with
        #       session-level timeouts; this is a direct socket-level timeout.
        #       See: https://requests.kennethreitz.org/en/master/user/advanced/#timeout
    ...
    def _call_requests(self, method: str, url: str, **kwargs):
        """Delegate HTTP calls to shared llm_manager.call_requests which handles
        compatibility with monkeypatched test fakes and timeout handling.
        """
