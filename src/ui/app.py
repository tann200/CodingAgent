...
class CodingAgentApp:
    def __init__(self, config: Optional[AppConfig] = None):
        ...
        self.orchestrator = Orchestrator(working_dir=self.config.working_dir, allow_external_working_dir=True)
        # Wire telemetry consumer if requested
        try:
            if getattr(self.config, 'telemetry_enabled', False) and TelemetryConsumer is not None:
                telemetry_path = self.config.telemetry_path or (Path(self.config.working_dir or '.') / 'telemetry.jsonl')
                try:
                    self.telemetry_consumer = TelemetryConsumer(self.event_bus, Path(telemetry_path))
        ...
        if enable_tui:
            ...
            try:
                ...
                if _il.find_spec('textual') is not None:
                    guilogger.info('Textual detected and ENABLE_TUI requested; launching TUI')
                    try:
                    ...
                    guilogger.info('Textual not found in environment; falling back to headless')
            except Exception:
