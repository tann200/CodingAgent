...
if not TEXTUAL_AVAILABLE:
    ...
    class TextualAppStub(TextualAppBase):
        def run(self) -> None:
            ...
            print("Alternatively, run the headless app shim via src.ui.app.CodingAgentApp")
    ...
    class CodingAgentTextualApp(App, TextualAppBase):
                ...
                with Container(id="main_area"):
                    ...
                    with Container(id="input_container"):
                        with Horizontal(id="input_status_bar"):
                            ...
                            self.provider_model_label = Static("Provider: None | Model: None", id="provider_model_info")
                            yield self.mode_label
                            yield self.provider_model_label
                        
                        self.input_widget = ChatInput(placeholder="Type prompt and press Enter...", id="chat_input")
                        yield self.input_widget
                        
                        with Horizontal(id="input_legend"):
                            yield Static("⌨️  ^O Settings | ^L Toggle Log | Enter Send", id="legend_info")
                            
                with Container(id="sidebar"):
                    with Container(id="sidebar_top"):
                        yield Static("📊 Context", classes="sidebar_title")
                        self.context_label = Static("Used: 0\nLimit: 128k\n0%", id="context_info")
                        yield self.context_label
                        
                        yield Static("📝 Task State", classes="sidebar_title")
                        self.task_state_label = Static("No active task", id="task_state_info")
        ...
        async def on_mount(self) -> None:
            ...
            try:
                ...
                if pm:
                    ...
                    from src.core.llm_manager import _ensure_provider_manager_initialized_sync
            ...
            try:
                eb = get_event_bus()
                if eb:
                    eb.subscribe('provider.status.changed', self._on_provider_status_changed)
                    eb.subscribe('provider.models.list', self._on_provider_models)
                    eb.subscribe('log.new', self._on_log_new)
                    eb.subscribe('model.usage', self._on_token_usage)
            ...
            try:
                ...
                if self.orchestrator.adapter:
                    ...
                    if hasattr(adapter, "provider") and isinstance(adapter.provider, dict):
                        self._current_provider = adapter.provider.get("name") or adapter.provider.get("type") or "None"
                    if hasattr(adapter, "models") and isinstance(adapter.models, list) and adapter.models:
        ...
        async def _refresh_provider_info(self) -> None:
            ...
            try:
                ...
                if providers:
                    ...
                    if not models and hasattr(self.orchestrator.adapter, 'get_models_from_api'):
                        try:
                            # Run in thread since it might be slow
                            def probe():
                                pm.get_models_from_api(prov)
                                # trigger UI refresh on bus
                                self.event_bus.publish('provider.models.list', {
                                    'provider': prov,
                                    'models': pm.get_cached_models(prov)
                                })
        ...
        def _update_provider_model_label(self):
            if self.provider_model_label:
                self.provider_model_label.update(f"Provider: {self._current_provider} | Model: {self._current_model}")
        ...
        def _on_log_new(self, payload) -> None:
            try:
                if self.sys_log:
                    self.sys_log.write(f"[{payload.get('timestamp')}] [{payload.get('level')}] {payload.get('message')}")
            except Exception:
                pass
        ...
        async def on_input_submitted(self, event: Input.Submitted) -> None:
            ...
            if isinstance(self.input_widget, ChatInput):
                if not self.input_widget.history or self.input_widget.history[-1] != raw_text:
        ...
        def _on_token_usage(self, payload) -> None:
            ...
            limit = payload.get('max_tokens', 128000)
            percent = (used / limit) * 100 if limit > 0 else 0
            self.context_label.update(f"Used: {used}\nLimit: {limit//1000}k\n{percent:.1f}%")
        ...
        def action_open_settings(self) -> None:
            try:
                ...
                try:
                    ...
                    providers = []
                self._settings_modal.open_with(providers, providers[0] if providers else None)
    ...
    class ConnectProviderModal(ModalScreen):
        ...
        def compose(self):
            with Container(id="provider_dialog"):
                ...
                self.provider_select = Select(options=opts, value=self.current, id="provider_select")
                yield self.provider_select
                yield Button("Connect", id="connect_btn", classes="provider_btn", variant="primary")
    ...
    class SelectModelModal(ModalScreen):
        ...
        def compose(self):
            with Container(id="model_dialog"):
                ...
                val = self.current if self.current in self.models else (self.models[0] if self.models else None)
                self.model_select = Select(options=opts, value=val, id="model_select")
                yield self.model_select
                yield Button("Select", id="select_btn", classes="model_btn", variant="primary")
    ...
    class SettingsModal(ModalScreen):
        ...
        def compose(self):
            with Container(id="settings_dialog"):
                ...
                yield Button("New Session", id="settings_new_session", classes="settings_btn")
                yield Button("Compact Session", id="settings_compact_session", classes="settings_btn")
                yield Button("Connect Provider", id="settings_connect_provider", classes="settings_btn")
                yield Button("Select Model", id="settings_select_model", classes="settings_btn")
                yield Button("Close", id="settings_close", classes="settings_btn", variant="error")
        ...
        def on_button_pressed(self, event: Button.Pressed) -> None:
            ...
            if id_ == 'settings_close':
                ...
                if hasattr(self.app, "output") and self.app.output:
                    self.app.output.write("[System] Compacting session... (Placeholder)")
                self.app.pop_screen()
            elif id_ == 'settings_connect_provider':
                self.app.pop_screen()
                self.app.push_screen(ConnectProviderModal(self.providers, getattr(self.app, "_current_provider", None)))
                ...
                try:
                    ...
                    pass
                self.app.push_screen(SelectModelModal(models, getattr(self.app, "_current_model", None)))

        def open_with(self, providers: List[str], current_provider: Optional[str] = None):
            self.providers = providers
