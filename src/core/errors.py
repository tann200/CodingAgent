...
_MESSAGE_PATTERNS: list[tuple[re.Pattern[str], ErrorCode]] = [
    ...
    (re.compile(r"context.{0,20}(overflow|length|window|limit|exceed|too long)"
    ...
    (re.compile(r"connection.{0,20}(refused|reset|error|failed|abort|closed)|"
                r"(503|502|504|500).{0,30}provider"),        ErrorCode.LLM_CONNECTION_FAILED),
    (re.compile(r"budget.{0,20}exceed|max.{0,10}tokens"),   ErrorCode.LLM_BUDGET_EXCEEDED),
    ...
    (re.compile(r"ssrf|private.{0,10}address|blocked.*host"), ErrorCode.TOOL_SSRF_BLOCKED),
