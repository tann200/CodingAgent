        ...
        try:
            if hasattr(adapter, "get_models_from_api") and callable(getattr(adapter, "get_models_from_api")):
                ...
                if isinstance(models_resp, dict):
                    ...
                    res["ok"] = bool(norm)
                    if not norm:
                        guilogger.warning(f"Startup: provider '{key}' returned no models from API")
                else:
                    # Unexpected shape - treat as not ok but include raw
                    res["models"] = models_resp
                    res["error"] = "unexpected_models_shape"
                    guilogger.warning(f"Startup: provider '{key}' returned unexpected models shape: {type(models_resp)}")
            else:
                # Fallback: try validate_connection
                if hasattr(adapter, "validate_connection") and callable(getattr(adapter, "validate_connection")):
                    ...
                    if not ok:
                        ...
                        guilogger.warning(f"Startup: provider '{key}' validate_connection failed")
                else:
