...
def _compile_full_graph():
    ...
    workflow.add_node("analysis", _validated("analysis", analysis_node))
    workflow.add_node("planning", _validated("planning", planning_node))
    workflow.add_node("plan_validator", _validated("plan_validator", plan_validator_node))
    workflow.add_node("execution", _validated("execution", execution_node))
    workflow.add_node("step_controller", _validated("step_controller", step_controller_node))
    workflow.add_node("verification", _validated("verification", verification_node))
    workflow.add_node("debug", _validated("debug", debug_node))
    workflow.add_node("memory_sync", _validated("memory_sync", memory_update_node))
    workflow.add_node("delegation", _validated("delegation", delegation_node))
    workflow.add_node("analyst_delegation", _validated("analyst_delegation", analyst_delegation_node))
    workflow.add_node("replan", _validated("replan", replan_node))
    workflow.add_node("evaluation", _validated("evaluation", evaluation_node))
    workflow.add_node("wait_for_user", _validated("wait_for_user", wait_for_user_node))
