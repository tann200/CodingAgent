...
from src.ui.events import RequestSystemSettings, SaveProviderCredentials, UpdateRoleModel
from src.ui.app import AgentApp
from src.ui.logging import get_logger

logger = get_logger("mock_engine")

PROVIDERS_JSON_PATH = Path(__file__).resolve().parent.parent / "core" / "providers.json"
...
class MockSimulationApp(AgentApp):
    ...
    def handle_request_settings(self, _: RequestSystemSettings) -> None:
        ...
        self.post_message(SystemSettingsLoaded(
    ...
    async def run_simulation(self):
        ...
        bus.publish("orchestrator.startup", {
            "time": time.time(),
        ...
        # Provider discovery
        bus.publish("provider.status.changed", {
            "provider": "local_lm_studio",
            "status": "connected",
        })
        bus.publish("provider.models.list", {
            "provider": "local_lm_studio",
            "models": ["qwen2.5-coder-14b", "deepseek-coder-v2", "llama-3-8b"],
        })
        bus.publish("model.routing", {
            "provider": "local_lm_studio",
        ...
        bus.publish("task.queue.updated", {
            "pending_count": 3,
            "queue_size": 3,
            "new_status": "queued",
        })
        ...
        bus.publish("role.transition", {
            "from_role": "system", "to_role": "lead_architect", "run_id": "mock-001",
        })
        await asyncio.sleep(0.3)

        bus.publish("token.budget.update", {"used": 800, "limit": 32000, "percent": 2.5})
        bus.publish("ui.notification", {
            "level": "info",
        ...
        t1 = _tid()
        bus.publish("tool.execute.start", {
        ...
        bus.publish("tool.execute.finish", {
        ...
        bus.publish("tool.execute.error", {
        ...
        bus.publish("tool.execute.start", {
            ...
            "title": "grep",
            "status": "in_progress",
            "rawInput": {"pattern": "get_event_bus|_subscribe", "path": "src/ui/", "include": "*.py"},
        })
        ...
        bus.publish("model.response", {
            "provider": "local_lm_studio",
            "model": "qwen2.5-coder-14b",
            "tokens": 347,
        })
        bus.publish("token.budget.update", {"used": 5200, "limit": 32000, "percent": 16.3})
        await asyncio.sleep(0.2)

        # plan.progress — ACP schema, step 1 of 4
        bus.publish("plan.progress", {
            "currentStep": 1,
            "totalSteps": 4,
        ...
        bus.publish("role.transition", {
            "from_role": "lead_architect", "to_role": "full_stack_engineer", "run_id": "mock-001",
        })
        await asyncio.sleep(0.3)
        ...
        bus.publish("token.budget.update", {"used": 9100, "limit": 32000, "percent": 28.4})
        ...
        bus.publish("tool.execute.start", {
            ...
            "title": "write_file",
            "status": "in_progress",
            "rawInput": {"path": "src/ui/mock_engine.py", "content": "<3 241 chars>"},
        })
        ...
        bus.publish("tool.execute.finish", {
            ...
            "status": "completed",
            "ok": True,
            "content": [{"text": "✓ Modified src/ui/mock_engine.py  [+142 / -31 lines]\n"}],
        })
        ...
        bus.publish("tool.execute.finish", {
            ...
            "status": "completed",
            "ok": True,
            "content": [{"text": "✓ Modified src/ui/core_bridge.py  [+1 / -1 lines]\n"}],
        })
        bus.publish("file.modified", {
            "path": "src/ui/core_bridge.py",
            "tool": "edit_file_atomic",
        ...
        bus.publish("token.budget.update", {"used": 19800, "limit": 32000, "percent": 61.9})
        await asyncio.sleep(0.2)
        ...
        bus.publish("role.transition", {
            "from_role": "full_stack_engineer", "to_role": "qa_lead", "run_id": "mock-001",
        })
        await asyncio.sleep(0.3)
        ...
        bus.publish("token.budget.update", {"used": 24600, "limit": 32000, "percent": 76.9})
        await asyncio.sleep(0.15)

        # session.health_alert (§4.5) — tests SessionHealthEvent handler
        bus.publish("session.health_alert", {
            "level": "warning",
        ...
        bus.publish("token.budget.update", {"used": 28900, "limit": 32000, "percent": 90.3})
        bus.publish("token.budget.warning", {"used": 28900, "limit": 32000, "percent": 90.3})
        await asyncio.sleep(0.15)

        # file.deleted — exercises _on_file_deleted in bridge
        bus.publish("file.deleted", {
        ...
        # ── Final response ─────────────────────────────────────────────────
        self.post_message(AgentFinalResponse(content=(
        ...
        bus.publish("token.budget.update", {"used": 29700, "limit": 32000, "percent": 92.8})
