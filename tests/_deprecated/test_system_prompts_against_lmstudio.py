...
def test_system_prompts_behaviour_lmstudio():
        ...
        try:
            ...
            if isinstance(body, dict):
                ...
                if isinstance(err, dict):
                    ...
                    if (
                        "failed to load model" in str(msg).lower()
                        or "failed to load" in str(msg).lower()
                    ):
                        return True
    ...
    if _is_model_load_error(res):
        pytest.xfail(
            f"LM Studio model not loaded or failed to load: {res.get('error')}. Load the configured model in LM Studio and re-run integration tests."
        )
    # otherwise expect ok
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "Produce steps to add a new tool to the orchestrator.",
        },
    ]
    ...
    if _is_model_load_error(pres):
        pytest.xfail(
            f"LM Studio model not loaded or failed to load: {pres.get('error')}. Load the configured model in LM Studio and re-run integration tests."
        )
