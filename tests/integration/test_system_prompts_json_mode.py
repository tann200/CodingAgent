...
def test_system_prompt_json_mode_and_tool_handover():
    ...
    try:
        ...
        system = load_system_prompt(None) or Path(
            "agent-brain/agents/coding_agent.md"
    ...
    try:
        r, used = post_with_retry(
            endpoints, payload, overall_timeout=120, per_request_timeout=60
        )
        ...
        pytest.skip(
            f"Endpoint not ready after retries: {e}\nDiagnostics:\n"
            + "\n".join(details)
        )
    ...
    assert parsed is not None and isinstance(parsed, dict), (
        f"Expected JSON-parsed dict, got: {data}"
    )
