...
class TestStepControllerFailedStepRetryRouting:
    def test_failed_step_routes_to_execution(self):
        ...
        assert result == "execution", f"Expected 'execution' for retry, got '{result}'"
...
class TestExecutionNodeStripsLLMInjectedApprovalKey:
...
class TestBashBlocksSedInplaceTarExtractUnzip:
    ...
    def test_sed_without_i_is_allowed(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result.get("status") != "error" or "in-place" not in result.get(
                "error", ""
            )
...
class TestEditByLineRangeLineEditsAndPathSafety:
    ...
    def test_registered_in_orchestrator(self, tmp_path):
        ...
        assert tool is not None, (
            "edit_by_line_range must be registered in example_registry"
        )
...
class TestPlanningNodeFallbackPlanOnLLMFailure:
    ...
    async def test_fallback_plan_on_llm_exception(self, tmp_path, monkeypatch):
        ...
        assert len(plan) >= 1, (
            "planning_node must return at least a 1-step fallback plan"
        )
...
class TestIndexedDirsCacheSkipsRedundantIndexing:
    @pytest.mark.asyncio
...
class TestPerceptionNodeSkipsSearchOnSubsequentRounds:
    @pytest.mark.asyncio
...
class TestProviderContextTokenBudgetBounds:
...
class TestAnalysisNodeRegexIdentifierExtraction:
    @pytest.mark.asyncio
    async def test_implementation_task_finds_real_identifier(
...
class TestWriteFileSetsRequiresSplitForLargeFiles:
...
class TestFileCacheLRUEvictionBeyondMaxSize:
...
class TestOrchestratorSessionReadFilesResetPerTask:
