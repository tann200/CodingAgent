...
def test_every_registered_tool_in_some_toolset():
    ...
    # canonical tool names from the registry: exclude dotted aliases and
    # short-form aliases configured in TOOL_ALIASES (these are user-facing
    # shortcuts that map to canonical names and need not appear in toolsets).
    canonical = [n for n in reg.list() if "." not in n and n not in TOOL_ALIASES]
