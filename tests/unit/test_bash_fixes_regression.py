...
class TestAnalyzeBashCommandCache:
    ...
    def test_same_command_returns_same_object(self):
        ...
        r1 = analyze_bash_command("ls -la")
        r2 = analyze_bash_command("ls -la")
        # With lru_cache, these should be the same object.
        assert r1 is r2

    def test_cache_info_populated_after_calls(self):
        from src.tools.bash_security import analyze_bash_command
        analyze_bash_command("echo hello world")
        analyze_bash_command("echo hello world")
        info = analyze_bash_command.cache_info()
        assert info.hits >= 1
...
class TestCheckBackgroundTask:
    ...
    def test_run_in_background_returns_pid(self, tmp_path):
        ...
        result = bash("python3 -c \"import time; time.sleep(5)\"", workdir=tmp_path, run_in_background=True)
...
class TestPlanningPreflight:
    ...
    def test_step_with_npm_install_gets_warning(self):
        ...
        steps = [
            {"id": "1", "description": "Run npm install to fetch dependencies", "tool": "bash"},
        ]
        ...
        for step in steps:
            ...
            for kw in restricted_keywords:
                if kw in desc:
                    step.setdefault("warnings", []).append(f"contains restricted command '{kw}'")
                    break
    ...
    def test_step_with_git_push_gets_warning(self):
        ...
        steps = [
            {"id": "1", "description": "git push origin main to deploy", "tool": "bash"},
        ]
        restricted_keywords = sorted(RESTRICTED_COMMANDS, key=len, reverse=True)
        for step in steps:
            desc = str(step.get("description", "")).lower()
    ...
    def test_safe_step_has_no_warning(self):
        ...
        steps = [
            {"id": "1", "description": "Run pytest to verify all tests pass", "tool": "run_tests"},
        ]
        restricted_keywords = sorted(RESTRICTED_COMMANDS, key=len, reverse=True)
        for step in steps:
            desc = str(step.get("description", "")).lower()
...
class TestBashReadonlySandboxed:
    ...
    def test_bash_readonly_calls_run_sandboxed(self, tmp_path):
        ...
        assert call_kwargs.get("network") is False or (
            len(call_args) > 1 and call_args[1] is False
        ) or "network" not in call_kwargs  # sandbox may not yet support network param
...
class TestCheckShellFlagsBlocked:
    ...
    def test_sed_inline_edit_blocked_in_bash(self, tmp_path):
        ...
        assert result["status"] == "error"
        assert "sed -i" in result["error"] or "in-place" in result["error"].lower() or "blocked" in result["error"].lower()
...
class TestRestrictedCommandsContents:
    ...
    def test_git_clone_triggers_requires_approval(self, tmp_path):
        ...
        result = bash("git clone https://github.com/example/repo /tmp/repo", workdir=tmp_path)
        # Either blocked by DANGEROUS_PATTERNS (if | appears somewhere) or requires_approval
        assert result["status"] in ("error", "requires_approval") or result.get("requires_approval") is True
