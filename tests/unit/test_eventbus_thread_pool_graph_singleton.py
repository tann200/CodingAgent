...
class TestEventBusWildcardCallbackDeduplication:
    ...
    def test_different_wildcard_and_specific_callbacks_both_fire(self):
        ...
        def wc(m):
            wildcard_calls.append(m)

        def sc(m):
            specific_calls.append(m)

        bus.subscribe_to_agent("*", wc)
        bus.subscribe_to_agent("agent1", sc)
        bus.publish_to_agent("agent1", "payload")
...
class TestAgentStateToolLastUsedAndFilesReadPresent:
...
class TestToolTimeoutThreadPoolExecutorFromDaemonThread:
    ...
    def test_threadpool_timeout_fires_from_daemon_thread(self):
        ...
        assert not t.is_alive(), (
            "Daemon thread must finish within 5s when timeout fires"
        )
...
class TestSandboxAstParsePythonContentBeforeWrite:
    ...
    def test_new_content_validated_via_ast_parse(self):
        ...
        # Variable was renamed new_py_content → new_content when JS/TS support was added
        assert "new_content" in source or "new_py_content" in source, (
            "C2 fix: execute_tool must extract new content from tool args for syntax validation"
        )
...
class TestAnalysisFastPathBypassForSimpleTasks:
...
class TestCompiledGraphSingletonCache:
    ...
    def test_get_compiled_graph_returns_same_object(self):
        ...
        assert g1 is g2, (
            "_get_compiled_graph() must return the same compiled graph object"
        )
...
class TestIndexedDirsMtimeBasedStalenessDetection:
...
class TestRunAgentOncePropagatesDebugAndRetryCounters:
...
class TestBashDangerousPatternWhitespaceNormalization:
