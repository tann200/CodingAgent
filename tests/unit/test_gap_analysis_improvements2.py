...
class TestSmall3ParallelToolsFlag:
    def _compute_simple_mode(
        self, model_tier: str, provider_supports_parallel_tools: bool
    ) -> bool:
    ...
    def test_small_no_parallel_tools_enters_simple_mode(self):
        result = self._compute_simple_mode(
            "small", provider_supports_parallel_tools=False
        )
        assert result is True  # SMALL is always simple mode

    def test_small_with_parallel_tools_stays_simple(
        self,
    ):  # Fixed: SMALL is always simple
        result = self._compute_simple_mode(
            "small", provider_supports_parallel_tools=True
        )
        assert result is True  # SMALL always uses simple mode

    def test_small_always_simple_mode(self):  # Changed from nano to small
        result = self._compute_simple_mode(
            "small", provider_supports_parallel_tools=True
        )
    ...
    def test_medium_not_simple_mode(self):
        result = self._compute_simple_mode(
            "medium", provider_supports_parallel_tools=False
        )
        assert result is False

    def test_frontier_not_simple_mode(self):
        result = self._compute_simple_mode(
            "frontier", provider_supports_parallel_tools=False
        )
        assert result is False
...
class TestSmall4ClarificationGuard:
    ...
    def test_longer_task_does_not_trigger(self):
        ...
        assert (
            self._should_clarify("the thing with the other thing and also this")
            is False
        )
...
class TestModelSpecificPartialFiles:
    ...
    def test_anthropic_small_is_short(self):
        ...
        assert len(lines) <= 25, (
            f"anthropic-small.md has {len(lines)} non-empty lines, expected ≤25"
        )
