...
class FakeToolRegistry:
    ...
    def get(self, name):
        if name == "search_code":
            return {
                "fn": lambda **kwargs: [
                    {"file_path": "src/a.py", "snippet": "def foo(): pass"}
                ]
            }
        if name == "find_symbol":
            return {
        ...
        if name == "find_references":
            return {
                "fn": lambda **kwargs: [
                    {"file_path": "src/c.py", "excerpt": "foo() called here"}
                ]
            }
        return None
...
class FakeOrch:
    def __init__(self):
        ...
        # BUG-FIX: provide mock adapter for perception_node
        self.adapter = type("FakeAdapter", (), {"chat": lambda *a, **k: {}})()
