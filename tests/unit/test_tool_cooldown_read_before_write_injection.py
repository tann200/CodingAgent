...
class TestAgentStateToolLastUsedAndFilesReadFieldTypes:
...
class TestExecutionNodeToolCooldownGapEnforcement:
    ...
    async def test_non_read_tools_not_subject_to_cooldown(self, tmp_path):
        ...
        state = _make_state(
            ...
            next_action={
                "name": "edit_file",
                "arguments": {"path": "auth.py", "content": "x=1"},
            },
...
class TestExecutionNodeReadBeforeWriteEnforcement:
    ...
    async def test_modifying_tool_blocked_when_files_read_empty(self, tmp_path):
        ...
        # Create the file so the read-before-write check applies
        (tmp_path / "auth.py").write_text("original content")
        ...
        state = _make_state(
            ...
            next_action={
                "name": "edit_file",
                "arguments": {"path": "auth.py", "content": "x"},
            },
...
class TestPlanValidatorInvalidPlanRoutesToPlanning:
    ...
    def test_action_failed_routes_to_planning(self):
        ...
        state = _make_state(
            action_failed=True, plan_validation={"valid": True}, rounds=0
        )
        result = should_after_plan_validator(state)
        assert result == "planning"
...
class TestBashBlocksAllSedInplaceForms:
    ...
    def test_bare_dash_i_blocked(self):
        ...
        assert (
            "in-place" in result.get("error", "").lower()
            or "sed" in result.get("error", "").lower()
        )
...
class TestGlobToolRejectsPathTraversal:
...
class TestPlanningNodeMaxTokensBudget:
...
class TestPerceptionNodeInjectedToolNameDetection:
...
class TestDelegationNodeInjectsResultsIntoMessageHistory:
    ...
    async def test_delegation_results_injected_into_history(self):
        ...
        assert (
            "auth_analysis" in combined
            or "auth module" in combined.lower()
            or "subagent" in combined.lower()
        )
