...
def test_orchestrator_emits_startup_and_model_events(monkeypatch, tmp_path):
    ...
    assert any(e[0] == 'startup' for e in events), f"Missing startup event, events: {events}"

    # assert background probe cached models event occurred
    assert any(e[0] == 'cached' for e in events) or any(e[0] == 'completed' for e in events), f"Missing cached/completed events, events: {events}"
