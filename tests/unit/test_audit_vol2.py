...
class TestStepControllerRouting:
...
class TestPlanningNodeOrchestratorFetch:
...
class TestContextBuilderCwdBug:
...
class TestExecutionNodeCallModel:
...
class TestDelegateTaskAsyncExecutor:
...
class TestTrajectoryLoggerThreadSafety:
    ...
    def test_trajectory_logger_has_no_thread_lock(self):
        ...
        has_lock = "Lock" in src or "lock" in src.lower() and "threading" in src.lower()
...
class TestVectorStoreSearchResults:
...
class TestFixedFindingsRegression:
    ...
    def test_new1_debug_node_uses_await_on_call_model(self):
        ...
        # Should contain 'await call_model', not bare 'resp = call_model'
        assert "await call_model" in src, (
            "NEW-1 regression: debug_node must use 'await call_model'"
        )
