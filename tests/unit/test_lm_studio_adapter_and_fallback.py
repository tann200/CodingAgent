...
def test_chat_returns_user_message_on_model_load_error(monkeypatch):
    ...
    body = {"error": {"message": 'Failed to load model "qwen/qwen3.5-9b". Error: insufficient system resources. requires approximately 8.89 GB'}}
...
def test_llm_manager_fallback(monkeypatch):
    ...
    class MockAdapter:
        ...
        def get_models_from_api(self):
            return {'models': [{'name': 'qwen3.5-9b', 'id': 'qwen/qwen3.5-9b'}, {'name': 'smallmodel','id':'small/smallv1'}]}
        ...
        def chat(self, messages, model=None, stream=False, format_json=False):
            ...
            return {'ok': True, 'message': {'role': 'assistant', 'content': 'ok from '+str(model)}}
    ...
    resp = asyncio.run(call_model(messages, provider='lm_studio', model='qwen/qwen3.5-9b', stream=False, format_json=False))
