...
from src.core.inference.adapters import lm_studio_adapter as lm_mod  # type: ignore[import]

importlib.reload(lm_mod)
from src.core.inference.adapters.lm_studio_adapter import LmStudioAdapter  # type: ignore[import]
...
def fake_get_models(self):
    call_count["n"] += 1
    print(
        f"[diag] LMStudioAdapter.get_models_from_api called #{call_count['n']}",
        flush=True,
    )
...
import src.core.llm_manager as lm  # type: ignore[import]
...
print(
    "[diag] Cached models for lm_studio:", pm.get_cached_models("lm_studio"), flush=True
)
...
print(
    "[diag] Adapter instantiated; static models in adapter:",
    getattr(ad, "models", None),
    flush=True,
)
...
time.sleep(1)
print(
    "[diag] Background probe likely ran; final fake call count:",
    call_count["n"],
    flush=True,
)
...
print(
    "[diag] First get_available_models call result:",
    asyncio.run(lm.get_available_models("", "", "lm_studio")),
    flush=True,
)
print(
    "[diag] Second get_available_models call result:",
    asyncio.run(lm.get_available_models("", "", "lm_studio")),
    flush=True,
)
